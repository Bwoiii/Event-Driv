...
const TransactionManagement = ({ products = [], setProducts, onPaymentCompleted}) => {
  ...
  const handleCashOnDeliverySubmit = (e) => {
    ...
    if (
      ...
      Swal.fire("Please fill in all the required fields for Cash on Delivery. Delivery.");
    } else {
  ...
  const handlePaymentCompleted = () => {
    if (!PaymentMethod || PaymentMethod === '') {
      Swal.fire("Please select a payment method before completing the payment.");
      return;
    }
    if (CashOnDeliverySelected && !cashOnDeliveryDetails.fullName.trim()) {
      Swal.fire("Please provide your full name for Cash on Delivery.");
      return;
    }
    if (CashOnDeliverySelected && !cashOnDeliveryDetails.shippingAddress.trim()) {
      Swal.fire("Please provide your shipping address for Cash on Delivery.");
      return;
    }
    if (CashOnDeliverySelected && !cashOnDeliveryDetails.contactNumber.trim()) {
      Swal.fire("Please provide your contact number for Cash on Delivery.");
      return;
          ...
          <Col>
            <Nav variant="tabs" className="mb-3" style={{ fontSize: '20px', padding: '10px' }}>
              <Nav.Item>
                ...
                <i className="fas fa-box-open"></i>Products</Nav.Link>
              </Nav.Item>
              <Nav.Item>
                    ...
                    <tbody>
                      {cart.map((product) => (
                        <tr key={product.id}>
                          ...
                          <td>
                            <Button  className='btn-red' variant='danger' onClick={() => removeFromCart(product.cartId)}>Remove</Button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
